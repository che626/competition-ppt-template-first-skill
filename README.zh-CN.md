...
本 Skill 推荐先完成下面的基础链路：先把文档和用户要求拆成清晰的叙事与视觉任务，再做每页对应的优质模板，而不是直接套深色背景和几个框。

1. 先整理报告、附件、截图、数据中能用的内容和待补材料，判断这是答辩、产品展示、技术汇报还是研究说明。
2. 先写清楚受众、要达成的判断、叙事路线，以及项目本身可以成为画面的产品、场景、材质、设备、人物、样本或证据。
3. 为每页写清楚：背景图是什么、主图是什么、有什么辅助视觉物件、真实图片放哪里、文字放哪里、哪些内容必须可编辑、这一页禁止出现什么。
4. 对封面、问题、方案、成果等视觉重点页，先生成或设计一张整页模板图；技术信息密集页也要有针对内容的安静底图或原生版式。
5. 模板确认后，将其拆成背景、真实证据图片、可编辑文字和少量可替换覆盖层。
