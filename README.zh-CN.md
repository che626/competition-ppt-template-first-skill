...
> 先理清要讲什么和手头有什么证据，再把关键内容做成可编辑 PPT。
...
本 Skill 推荐先完成下面的基础链路，材料越完整，链路就可以做得越严谨：

1. 先整理报告、附件、截图、数据中能用的内容和待补材料。
2. 写清楚每页要讲什么、需要什么证据，以及哪些位置要留给真实图片。
3. 对封面、问题、方案、成果等视觉重点页，先生成或设计一张整页模板图；技术信息密集页可使用更安静的版式。
4. 模板确认后，将其拆成背景、真实证据图片、可编辑文字和少量可替换覆盖层。
5. 导出重点页面检查文字、图片和层级，按反馈修改并归档满意版本。

## 两种使用深度

| 方式 | 适合情况 | 主要动作 |
| --- | --- | --- |
| `基础流程` | 只有赛题、简介、少量截图或已有 PPT。 | 整理素材、规划页面、先做一张代表页、再制作可编辑 PPT 并检查可读性。 |
| `文档溯源流程` | 有作品报告、论文、数据表、获奖证明或较完整技术资料。 | 在基础流程上补充来源清单、事实登记和重点页面的证据对应。 |

两种方式的视觉方法相同。文档溯源流程主要用于有数据、模型对比、奖项、技术指标等需要经得起追问的页面，不要求为了走流程而给每句普通叙述都建立复杂表格。
...
对于数据、模型对比、获奖、技术指标等关键结论，建议绑定事实编号与文档定位，例如“报告 S-01，第 4.2 节”或“数据表 S-03，Metrics 工作表 B8:F12”。资料未定的内容要保留为可替换区域或明确标注为未来计划，不要为了排版补造事实。
...
先整理可用事实、PPT 目录、每页放什么和需要哪些真实图片；
再生成一张代表页模板图，确认风格后再批量制作。
```
