...
Complete `templates/deck-style-tokens.md`, then choose a dominant tone, one support tone, one sharp accent, type hierarchy, texture/material motif, title behavior, image treatment, spacing logic, and light/mid/dark distribution. Explain why this system belongs to the subject. Do not select a generic deep-blue technology system by default. Then map template intensity to the slide:
...
Render the actual PPT to images when possible. Check the changed and high-risk pages against `references/quality-gates.md`, then complete `templates/rendered-visual-critique.md` with composition, hierarchy, information-density, and typography findings. Fix every open major issue, re-render the affected page, then archive versions the user has confirmed.
...
- Its rendered visual critique has no open major composition, hierarchy, density, or typography issue.
