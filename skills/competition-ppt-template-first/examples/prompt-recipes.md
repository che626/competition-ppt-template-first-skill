...
Before creating the deck, write a content-to-deck analysis: deck type, audience decision, narrative route,
...
First initialize a report-grounded workspace. Then split the documents into the audience decision,
narrative material, proof material, visual material, and gaps; record this in content-analysis.md.
Give each document a source ID, extract verified facts and evidence IDs with exact section/page/table locators,
and create a slide-source map. For every proposed slide, show: the judge question, page conclusion,
fact IDs, real evidence, background scene, primary visual, supporting visual ingredients, exact text/image zones,
and transition to the next page. Do not default to deep-blue AI style. Only after I confirm this route should you
generate the representative template.
```
...
will be used and where they will sit. State the page's background scene, screenshot crop focus, supporting visual
ingredients, and editable text zones. Generate a quiet template around their required aspect ratios,
...
## 5. Build a product showcase from a product brief

```text
$competition-ppt-template-first
Read [product brief], [product photos / prototype renders], [interface screenshots], and [reference decks].
Before making any slide, decide whether the deck is a product showcase, competition defense, or technical review.
Give me a content analysis that states the audience decision, user scenario, product promise, product evidence,
visual ingredients, page route, and questions. Then give every page a template contract: the background scene,
dominant product or interface visual, supporting objects/details, exact screenshot/photo slot, text zones, contrast
mode, and forbidden generic patterns. Use the real product as the largest visual wherever possible.
```
